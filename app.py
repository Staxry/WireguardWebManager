...
@app.route('/delete_client/<client_name>', methods=['GET', 'POST'])
@login_required
def delete_client(client_id=None, client_name=None):
    ...
    try:
        ...
        # Находим клиента по ID или имени
        ...
        if client_id is not None:
            ...
            if client_to_delete is None:
                error_msg = f'Клиент "{client_name}" не найден'
                flash(error_msg)
                if request.method == 'POST':
                    return jsonify({'success': False, 'message': error_msg}), 404
                return redirect(url_for('index'))
        else:
            error_msg = 'Не указан ID или имя клиента'
            flash(error_msg)
            if request.method == 'POST':
                return jsonify({'success': False, 'message': error_msg}), 400
            return redirect(url_for('index'))
        
        client_name = client_to_delete['name']
        ...
        # 3. Пытаемся удалить клиента через оригинальный скрипт
        try:
...
'''
                ...
                try:
                    ...
                    if result.returncode == 0:
                        print(f"Клиент {client_name} удален через оригинальный скрипт")
            ...
            print(f"Не удалось удалить через скрипт: {e}")
        
        # 4. Перезагружаем конфигурацию WireGuard
        reload_cmd = f'wg-quick down {WIREGUARD_INTERFACE} && wg-quick up {WIREGUARD_INTERFACE}'
        stdout, stderr, code = run_command(reload_cmd)
        
        if code != 0:
            print(f"Предупреждение при перезагрузке WireGuard: {stderr}")
        ...
        if request.method == 'POST':
            return jsonify({'success': True, 'message': f'Клиент {client_name} успешно удален с сервера'})
        else:
        ...
        if request.method == 'POST':
            return jsonify({'success': False, 'message': f'Ошибка удаления клиента: {str(e)}'}), 500
        else:
...
def check_session_timeout():
    if (request.endpoint not in ['login', 'static'] and 
